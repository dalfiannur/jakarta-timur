...
import {DetailModal, DetailModalRef} from "@/app/(main)/satuan-unit-kerja/DetailModal";
                ...
                <div className="grid grid-cols-2 gap-6">
                    {items.map((item, index) => (
                        <div key={index}
                            ...
                            <button onClick={() => modalRef.current?.open(item)} className="w-20 h-20 relative">
                                <Image src={item.image} alt={item.areaName} fill className="object-cover rounded-xl"/>
                            </button>
                            <button onClick={() => modalRef.current?.open(item)} className="flex-1 grid gap-2 text-left">
                            ...
                            <Link href={`/satuan-unit-kerja/kecamatan/${item.slug}`} className="text-pink-500 z-30">
